<?php
/*******************************************************************************
* BBCode Google Web Fonts Copyright 2012, Markus Kress - Kress.IT						       *
********************************************************************************
* KitSitemap.english.php													   *
********************************************************************************
* License http://creativecommons.org/licenses/by-sa/3.0/deed.de CC BY-SA 	   *
* Support for this software  http://kress.it and							   *
* http://custom.simplemachines.org/mods/index.php?mod=3478					   *
*******************************************************************************/

$txt['kitbbcodegfont_mod'] = 'BBcode Google Web Fonts';
$txt['kitbbcodegfont_guest'] = 'Show/Hide Google Web Fonts to guests';
$txt['kitbbcodegfont_guest_show'] = 'Show Google Web Fonts to guests';
$txt['kitbbcodegfont_guest_hide'] = 'Hide Google Web Fonts to guests (faster page load / SEO)';

$txt['kitbbcodegfont_mod_charsets'] = 'Extended charsets (use carefully only if needed - increases load time)';
$txt['kitbbcodegfont_charset_latin_ext'] = 'latin special charset';
$txt['kitbbcodegfont_charset_greek'] = 'greek charset';
$txt['kitbbcodegfont_charset_cyrillic'] = 'cyrillic charset';

$txt['kitbbcodegfont_bbc_gfont'] = 'Google Web Fonts';

?>